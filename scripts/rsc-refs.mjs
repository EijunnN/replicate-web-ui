...
//
//   node rsc-refs.mjs <capture/html/page.html | https://example.com/page> [payload.txt]
//
...
for (const [, id, moduleId, chunks, exportName] of refs) {
  ...
  console.log(`$L${id} -> module ${moduleId} export ${exportName || "(default)"}${files.length ? `  [${files.join(", ")}]` : ""}`);
}
